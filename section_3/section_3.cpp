...
void appendMatrix3(int arr[], int size, int xloc);
void displayMatrix();
    ...
    for(int i=0;i<data1_Index;i++){
        ...
        //else if(i==data1_Index-1){}                       // skip last index
            ...
            if(i!=data1_Index-1){
                ...
                xcoord1Jump[difference[0]-1] = i+1;
            }
        }
    }
    cout << "\nJump Index > { " ;
    ...
    for(int i=0;i<data2_Index;i++){
        ...
        //else if(i==data1_Index-1){}                       // skip last index
            ...
            if(i!=data2_Index-1){
                ...
                xcoord2Jump[difference[1]-1] = i+1;
            }
        }
    }
    cout << "\nJump Index > { " ;
...
if(data3_Index>0){                                          // not empty set
    for(int i=0;i<data3_Index;i++){
        static int indx = 0;
        static int placeIndex = 0;                          // static int, when running in loop, it will only declare once and not reset the value
        if(data3[i][0]==0){                                 //filter out (0, 0)
            cout << "zero!!" << endl;
            }      
        //else if(i==data1_Index-1){}                       // skip last index
        else if(data3[i+1][0]-data3[i][0]<threshold){       // same value
            indx = round(static_cast<double>(data3[i][0])/100.0)-1;
            //cout << indx << endl;
            xcoord3[indx]++;
        }
        else{                                               // different value, jump
            ...
            if(i!=data3_Index-1){
                difference[2]++;          // avoid appending difference when approaching last index, lastIndex+1 does not exist therefore should not count as one difference
                cout << "found 1 difference! current difference: " << difference[2] << ", current index: " << i << endl;
                xcoord2Jump[difference[2]-1] = i+1;
            }
        }
    }
    cout << "\nJump Index > { " ;
...
for(int i=0;i<xcoord3[1];i++){
    x200C[i]=data3[i+1+xcoord3[0]][1];
}
for(int i=0;i<xcoord3[2];i++){
    x300C[i]=data3[i+1+xcoord3[0]+xcoord3[1]][1];
}
for(int i=0;i<xcoord3[3];i++){
    x400C[i]=data3[i+1+xcoord3[0]+xcoord3[1]+xcoord3[2]][1];
}
for(int i=0;i<xcoord3[4];i++){
    x500C[i]=data3[i+1+xcoord3[0]+xcoord3[1]+xcoord3[2]+xcoord3[3]][1];
}
for(int i=0;i<xcoord3[5];i++){
    x600C[i]=data3[i+1+xcoord3[0]+xcoord3[1]+xcoord3[2]+xcoord3[3]+xcoord3[4]][1];
}
}
...
appendMatrix3(x100C, xcoord3[0], 0);
appendMatrix3(x200C, xcoord3[1], 1);
appendMatrix3(x300C, xcoord3[2], 2);
appendMatrix3(x400C, xcoord3[3], 3);
appendMatrix3(x500C, xcoord3[4], 4);
appendMatrix3(x600C, xcoord3[5], 5);
}
...
cout << "\n\ndata3 y-coord: ";
cout << "\nx100C: ";
for(int i=0;i<xcoord3[0];i++){              // get xcoord y-coordinate for raw3
    cout << x100B[i] << " ";                 // x location = 100
}
cout << "\nx200C: ";
for(int i=0;i<xcoord3[1];i++){
...
void displayMatrix(){
    ...
    cout << "------------- M a t r i x   d a t a   f o r   r a w 3 -----------------------\n" << endl;
    cout << "\tR1\tR2\tR3\tR4\tR5\tNS" << endl;
    for(int j=0;j<6;j++){
        if(j<4){
            cout << "D\t";
